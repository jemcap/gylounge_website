...
const monthKeyToUtcDate = (monthKey: string) => new Date(`${monthKey}-01T00:00:00.000Z`)

const getMonthStartFromDateKey = (dateKey: string) => monthKeyToUtcDate(dateKey.slice(0, 7))
...
const getInitialSelection = (
  ...
  const firstBookableLocation =
    locations.find((location) => (locationDateMap.get(location.id) ?? []).length > 0) ??
    locations[0]
...
export function BookingForm({
  ...
  const locationDateMap = useMemo(() => {
    ...
    for (const slot of slots) {
      ...
      const existingDate = locationDates.find((entry) => entry.date === slot.date)

      if (existingDate) {
        existingDate.slots = [...existingDate.slots, slot].sort(
          (left, right) => getSlotSortValue(left.startTime) - getSlotSortValue(right.startTime),
        )
      } else {
        locationDates.push({
          date: slot.date,
        ...
        locationDates.sort((left, right) => left.date.localeCompare(right.date))
      }
  ...
  const [clientFeedback, setClientFeedback] = useState<BookingFeedback | null>(null)
  ...
  const canSubmit = Boolean(selectedLocationId && selectedDate && selectedSlotId && hasSelectableSlots)
  const activeMonthKey = toMonthKey(visibleMonth)
  const canGoToPreviousMonth = monthKeys.indexOf(activeMonthKey) > 0
  const canGoToNextMonth =
    monthKeys.length > 0 && monthKeys.indexOf(activeMonthKey) < monthKeys.length - 1

  const selectDate = (dateKey: string, shouldValidate = true) => {
    const nextDateEntry = selectedLocationDates.find((entry) => entry.date === dateKey)
    const nextSlot = nextDateEntry?.slots[0]

    setSelectedDate(dateKey)
    setVisibleMonth(getMonthStartFromDateKey(dateKey))
    setValue("slotId", nextSlot?.id ?? "", { shouldDirty: true, shouldValidate })
  }
  ...
  const selectLocation = (locationId: string) => {
    ...
    setValue("locationId", locationId, { shouldDirty: true, shouldValidate: true })
    ...
    setValue("slotId", nextSlot?.id ?? "", { shouldDirty: true, shouldValidate: true })
  }
  ...
  return (
    ...
    >
      ...
      <form noValidate onSubmit={onSubmit} className="space-y-2">
            ...
            <div className="grid gap-5 lg:grid-cols-[minmax(0,0.75fr)_minmax(0,1.25fr)]">
              <div className="rounded-[1.75rem] p-3">
                <div className="mb-4 flex items-center justify-between gap-3">
                  ...
                  <p className="text-sm font-semibold text-[#261B07]">{formatMonthLabel(visibleMonth)}</p>
                  <button
                    type="button"
                    onClick={() => stepMonth(1)}
                ...
                {!selectedLocationDates.length ? (
                  <p className="mt-4 text-sm text-[#8b2e2a]">
                    No available booking dates have been configured for this location yet.
                  </p>
                ) : null}
              </div>
              ...
              <div className="rounded-[1.75rem]  p-4">
                ...
                {!selectedDateEntry ? (
                  <p className="mt-4 text-sm text-[#5f5240]">
                    Select one of the highlighted dates to see the time slots for this location.
                  </p>
                ) : null}
              </div>
            </div>

            {slotErrorMessage ? <p className={errorTextClass}>{slotErrorMessage}</p> : null}
          </div>
        </section>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className={helperTextClass}>
                We use these details to verify your membership and send booking confirmations.
              </p>
            </div>
        ...
        <Button type="submit" className="w-full justify-center py-3 text-base" disabled={isSubmitting || !canSubmit}>
